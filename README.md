...
双击后会打开命令行进行中文交互（别问我为什么不用图形界面简直脱裤子放屁放的还比穿了更臭`其实就是信息太乱了 本身是个不用动脑子一步一步走的活非得让小白费脑子看那个花里胡哨各种设置的界面qaq`）。发布包内置运行环境 点击就能用
...
4. App 会打开命令行（`就是那个黑色框框`）。按中文菜单选登录方式，粘贴 BV/AV、链接或整段分享
   文字，再选分p、画质、音频和无音频视频（想下什么下什么咯）。
6. 下载过程中让你输密码你就输入mac开机密码
7. 下载结束会自动打开 `~/Downloads/b站downloader/`。
8. `下载功能可以支持音频格式选择 视频画质选择 单独导出视频和音频`
